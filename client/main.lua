...
local function SendNotification(message, type, duration)
    if Config.Notify.system == "esx" then
        ESX.ShowNotification(message, type or 'success', duration)
...
RegisterNUICallback('startTest', function(data, cb)
    ESX.TriggerServerCallback('driving-school:server:checkMoney', function(hasMoney)
        if hasMoney then
            cb({ success = true })
        else
...
local function hasPassedTheoryTest()
    local passed = false
    ESX.TriggerServerCallback('driving-school:server:hasPassedTheory', function(result)
        passed = result
        testPassed = result
    end)

    local timeout = 0
    while not passed and timeout < 20 do
        Wait(100)
        timeout = timeout + 1
    end
...
local function startDrivingTest(licenseType)
    ...
    ESX.TriggerServerCallback('driving-school:server:checkMoney', function(hasMoney)
        if hasMoney then
            ...
            ESX.Game.SpawnVehicle(vehicleModel, startPos, startPos.w, function(vehicle)
